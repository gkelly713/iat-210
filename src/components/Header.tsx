...
const Header: React.FC<HeaderProps> = ({ onNavigate }) => {
  ...
  const sections = [
    ...
    { id: "game-world", label: "Game World" },
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-sm border-b border-slate-200 shadow-sm">
      <div className="container py-4 max-w-4xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-primary">Wordscape Wanderer</h1>
          </div>
        ...
        <div className={cn(
          "md:hidden",
          mobileMenuOpen 
            ? "max-h-96 opacity-100 transition-all duration-300 ease-in" 
          ...
          <nav className="flex flex-col pt-2 pb-4 space-y-2">
            {sections.map((section) => (
              <button
                key={section.id}
                onClick={() => handleNavClick(section.id)}
                className="px-2 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded"
              >
                {section.label}
              </button>
            ))}
